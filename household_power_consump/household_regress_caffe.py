...
def solver(train_net_path, test_net_path):
    ...
    s.test_interval = 512  # Test after every 1000 training iterations.
    s.test_iter.append(512) # Test 128 "batches" each time we test.

	# 500 Epochs --> (500 Epoch * 1503 # Samples)/(128 Batch Size) = Iterations
    s.max_iter = 320710      # # of times to update the net (training iterations)
...
def mlp(inputfile, batch_size):
    ...
    net.fc1 = cl.InnerProduct(net.data, num_output=64, weight_filler=dict(type='xavier'))
    net.relu1 = cl.ReLU(net.fc1, in_place=True)
    net.fc2 = cl.InnerProduct(net.relu1, num_output=512, weight_filler=dict(type='xavier'))
    net.relu2 = cl.ReLU(net.fc2, in_place=True)
    net.fc3 = cl.InnerProduct(net.relu2, num_output=1024, weight_filler=dict(type='xavier'))
    net.relu3 = cl.ReLU(net.fc3, in_place=True)
    net.fc4 = cl.InnerProduct(net.relu3, num_output=128, weight_filler=dict(type='xavier'))
    net.relu4 = cl.ReLU(net.fc4, in_place=True)
    net.fc5 = cl.InnerProduct(net.relu4, num_output=1, weight_filler=dict(type='xavier'))
    net.loss = cl.EuclideanLoss(net.fc5, net.label)
...
dirname = os.path.abspath('./household')
...
train_net_path = 'household/mlp_auto_train.prototxt'
with open(train_net_path, 'w') as f:
    f.write(str(mlp('household/train.txt', BATCH_SIZE)))

f.close();
	
test_net_path = 'household/mlp_auto_test.prototxt'
with open(test_net_path, 'w') as f:
    f.write(str(mlp('household/test.txt', BATCH_SIZE)))

f.close();
	
solver_path = 'household_mlp_solver.prototxt'
